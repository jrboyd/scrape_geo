...
library(data.table)
gse = "GSE54013" #will be combined with GSE19465
chara = list("cell type")
mat = scrape_geo(GSE_id = gse, 
                 do_srr = TRUE, debug = F, key_str = c("Title", "Source name", rep("Chara", length(chara))), 
                 key_idx = append(list(5, 5), chara))#, "cell line", "chip_antibody", "donor_id", "donor_sex"))
mat = cbind(mat, rownames(mat))
dt = as.data.table(mat)
colnames(dt) = c("Title", "Source name", unlist(chara), "srr", "gsm")

odir = paste0(gse, "_Runx2_in_MC3T3_timecourse")
dir.create(odir)
setwd(odir)
...
dt[, cell := "MC3T3"]
dt[, rep := "rep1"]
dt[, treatment := tstrsplit(Title, "_", keep = 2)]
dt[, chip := "Runx2"]
dt[is.na(treatment), chip := "input"]
dt[is.na(treatment), treatment := "none"]
dt[, file_name := paste0(cell, "_", chip, "-", treatment, "_", rep, ".fastq" )]
dt[, file_name := sub("-none_", "_", file_name)]
...
qc_dt = dt[, .(file_name)]
qc_dt[, c("cell", "chip", "rep") := tstrsplit(file_name, "[\\._]", keep = 1:3)]
...
top_dt = data.table(lines = c(paste0("in=", getwd()), 
                              paste0("out=", "/slipstream/galaxy/uploads/working/qc_framework/output_", odir)))
qc_f = paste0("qc_config_", odir, ".csv")
