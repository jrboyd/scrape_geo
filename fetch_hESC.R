...
library(data.table)
gsm_dt = fread("hESC_gsm.txt", header = FALSE)
colnames(gsm_dt) = c("cell", "mark", "gsm", "medium", "ab", "rep")
gse = "hESC" #will be combined with GSE19465
chara = list("cell type")
mat = scrape_geo(GSE_id = gse, 
                 gsm_override = gsm_dt$gsm,
                 # gsm_todo = "GSM537640", 
                 do_srr = TRUE, debug = F, 
                 key_str = c("Title", rep("Chara", length(chara))), 
                 key_idx = append(list(5), chara))#, "cell line", "chip_antibody", "donor_id", "donor_sex"))

gse_mat = scrape_gse(gsm_dt$gsm)
mat = cbind(mat, rownames(mat))
dt = as.data.table(mat)
dt$V2 = NULL
colnames(dt) = c("description", "srr", "gsm")
dt = merge(dt, gsm_dt)
# mat = mat[,c(1:2, 5:6)]
...
tmp = dt[, .(cell, mark, rep, gsm, srr)]
tmp = merge(tmp, gse_mat)
fwrite(tmp[, .(cell, mark, rep, gse, gsm, srr)], "hESC_supplemental.txt")
# colnames(mat) = c("id", chara, "srr", "gsm")
# library(data.table)
# dt = as.data.table(mat)
# for(ch in chara){
#   dt[[ch]] = sub(".+: ", "", dt[[ch]])
...
system("qstat")
dir()
qc_dt = dt[, .(file_name, cell, mark, rep)]
...
top_dt = data.table(lines = c(paste0("in=", getwd()), paste0("out=", "/slipstream/galaxy/uploads/working/qc_framework/output_", gse, "_court")))
