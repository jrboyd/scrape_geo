...
odir = paste0("fastqs_", gse, "_Ikaros_matB")
dir.create(odir)
...
dt[, c("mouse") := sub(".+pro-B ", "", title)]
dt[mark == "ChIP", mark := "input"]
dt[, stage := "proB"]
...
dt[, file_name := paste(paste("Busslinger", stage, status, mark, rep, sep = "_"), mouse, "fastq", sep = ".")]
